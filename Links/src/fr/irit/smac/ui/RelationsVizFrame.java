	...
	public void updateDesc(TreePath[] treePaths) {
		...
		if (treePaths != null) {
			for (int i = 0; i < treePaths.length; i++) {
				...
				case 3:
					if (treePaths[i].getPath()[1].toString().contains("RELATIONS")) {
						Relation r = this.linksWindow.getSnapshotsCollection().getRelation(
								treePaths[i].getPath()[2].toString(), this.linksWindow.getCurrentSnapNumber());
						if (r.getAttributes() != null)
							for (String str : r.getAttributes().keySet())
								for (Attribute t : r.getAttributes().get(str)) {
									this.toLook.add(new DrawableAttribute(DrawableAttribute.Type.RELATION, r.getName(),
											str, t));
								}
					}
					break;
